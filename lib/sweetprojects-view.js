  ...
  openProjectSettings(e){
    ...
    var groups = [
      ...
      },
      {
          ...
          label: 'Remote FTP',
          inputs: [
            {name: 'protocol', label: 'Protocol', value: '', placeholder: '', type: 'select', options: [
              {value: 'ftp', label: 'FTP'}, {value: 'sftp', label: 'SFTP'}, {value: 'n', label: 'NOT'}
            ]},
            {name: 'host', label: 'Host', value: '', placeholder: '', type: 'text', condition: 'protocol=ftp|protocol=sftp&port=20'},
            {name: 'port', label: 'Port', value: '21', placeholder: '', type: 'text'},
            {name: 'user', label: 'User', value: '', placeholder: '', type: 'text'},
            {name: 'pass', label: 'Password', value: '', placeholder: '', type: 'text'},
            {name: 'promptForPass', label: 'Protocol', value: '', placeholder: '', type: 'text'},
            {name: 'remote', label: 'Remote Dir', value: '/', placeholder: '', type: 'text'},
            {name: 'secure', label: 'Secure', value: 'control', placeholder: '', type: 'text'},
        ...
        inputelem.addEventListener('input', function(){
          ...
          for(var i in group.inputs){
            if(group.inputs[i].condition){
              ...
              /// cheap condition, supports = only
              if(false){
                var regex = /^([\w]*)([=<>]{1,2})(.*)$/;
                var parts = group.inputs[i].condition.split('=');
                var value = parts[0];
                group.inputs[i].elem.parentNode.style.display = (values[parts[0]] == parts[1]) ? 'block' : 'none';
              }
            }
          }
