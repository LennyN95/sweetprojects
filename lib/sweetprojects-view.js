  ...
  openProjectSettings(e){
    ...
    var groups = [
      ...
      },
      {
          ...
          inputs: [
            ...
            {name: 'port', label: 'Port', value: '21', placeholder: '', type: 'number'},
            {name: 'user', label: 'User', value: '', placeholder: '', type: 'text'},
            {name: 'pass', label: 'Password', value: '', placeholder: '', type: 'password'},
    ...
    for(var j in groups){
      ...
      for(var i in group.inputs){
        ...
        var value;
        try{
          value = project.getValue(group.package, input.name);
        }catch(e){
          value = input.value;
        }
    ...
    for(var j in groups){
      for(var i in groups[j].inputs){
        ...
        input.elem.dispatchEvent(event);
      }
    }
